...
public class ReservationController extends BaseController<ReservationService, ReservationRepository, Reservation> {
    ...
    @PutMapping("/{id}")
    public List<Reservation> myReservations(@AuthenticationPrincipal UserDetails userDetails) throws EntityNotFoundServiceException {
        return service.findUserReservations(usersService.get(userDetails));
    }

    @Transactional
    @PutMapping("/{id}")
    public void confirmReservation(@PathVariable("id") Long id) throws EntityNotFoundServiceException {
